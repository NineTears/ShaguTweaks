...
module.enable = function(self)    
    ...
    local function updateExp(mouseover)
        ...
        if playerlevel < 60 then
            if (not mouseover) then              
                if IsResting() then                    
                    if exh_perc > 0 then
                        exp.expstring:SetText(exh .. " (" .. exh_perc.."%) 精力充沛")
                    end
                else
                ...
                if (exh == 0) then
                    exp.expstring:SetText("等级 "..playerlevel.." - "..remaining.." ("..remaining_perc.."%) 剩余")            
                else
                    exp.expstring:SetText("等级 "..playerlevel.." - "..remaining.." ("..remaining_perc.."%) 剩余 - "..exh.." ("..exh_perc.."%) 精力充沛")
                end
            end
        end
    ...
    local function updateRep()
        ...
        if name then -- watching a faction
            exp.repstring:SetText(name .. " (" .. repvalues[standing] .. ") " .. percentFloor .. "% - "  .. remaining .. " 剩余")
        else
